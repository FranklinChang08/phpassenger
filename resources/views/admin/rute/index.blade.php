<x-admin-layout>
    <div
        ...
        <h1 class="text-gray-800 font-bold">Rute List</h1>
    ...
    <div class="flex items-center gap-6 my-4">
        <a href="{{ route('rute.create') }}"
            class="font-bold bg-blue-600 px-4 py-2 rounded-md text-white hover:bg-blue-700">Tambah Rute</a>
        <form action="{{ route('rute.index') }}" method="GET" class="flex items-center rounded-lg gap-2">
            <input type="text" name="search" placeholder="Cari rute...." value="{{ request('search') }}"
    ...
    <table class="table-auto w-full mt-4 bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <tbody>
            @if ($rute && $rute->count() > 0)
                @foreach ($rute as $data)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 border-b text-gray-700">{{ $data->id_rute }}</td>
                        ...
                        <td class="px-4 py-3 border-b text-gray-700 flex gap-4">
                            <a href="{{ route('rute.show', $data->id_rute) }}"
                                class="text-blue-600 hover:text-blue-800 font-semibold">
                                Edit
                            </a>
                            <form action="{{ route('rute.destroy', $data->id_rute) }}" method="POST"
                                class="inline-block" onsubmit="return confirm('Yakin ingin menghapus rute ini?')">
                                @csrf
                                @method('DELETE')
