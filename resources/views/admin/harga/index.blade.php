<x-admin-layout>
    <div class="hargaList">
        <div
            ...
            <h1 class="text-gray-800 font-bold">Harga List</h1>
        ...
        <div class="flex items-center gap-6 my-4">
            <button onclick="togglePartial()" class="font-bold bg-blue-600 px-4 py-2 rounded-md text-white">Tambah
                Form</button>
            <form action="{{ route('harga.index') }}" method="GET" class="flex items-center rounded-lg gap-2">
                <input type="text" name="search" placeholder="Cari Harga..." value="{{ request('search') }}"
        ...
        <table class="table-auto w-full mt-4 bg-white rounded-lg shadow-md overflow-hidden">
            <thead class="bg-gray-100">
                <tr class="text-left">
                    ...
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Harga</th>
            ...
            <tbody>
                    ...
                    @foreach ($harga as $data)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 border-b text-gray-700">
                                {{ 'Rp ' . number_format($data->harga, 2, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-3 border-b text-gray-700">
                                <a href="{{ route('harga.show', $data->id_harga) }}"
                                    class="text-blue-600 hover:text-blue-800 font-semibold mr-3">
                                    Edit
                                </a>
                                <form action="{{ route('harga.destroy', $data->id_harga) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Yakin ingin menghapus harga ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @else
                    <div class="flex items-center p-4 mb-4 text-sm text-red-800 bg-red-100 rounded-lg" role="alert">
                        ...
                        <span class="font-medium mr-2">Warning:</span>Data harga tidak tersedia.
                    </div>
                @endif
            </tbody>
    </div>
