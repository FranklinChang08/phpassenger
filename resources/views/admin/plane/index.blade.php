<x-admin-layout>
    <div class="PlaneList">
        <div
            ...
            <h1 class="text-gray-800 font-bold">Plane List</h1>
        ...
        <div class="flex items-center gap-6 my-4">
            <a href="{{ route('plane.create') }}"
                class="font-bold bg-blue-600 px-4 py-2 rounded-md text-white hover:bg-blue-700">Tambah Plane</a>
            <form action="{{ route('plane.index') }}" method="GET" class="flex items-center rounded-lg gap-2">
                <input type="text" name="search" placeholder="Cari plane..." value="{{ request('search') }}"
        ...
        <table class="table-auto w-full mt-4 bg-white rounded-lg shadow-md overflow-hidden">
            <thead class="bg-gray-100">
                <tr class="text-left">
                    ...
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Nama Maskapai</th>
            ...
            <tbody>
                @if ($plane->count() > 0)
                    ...
                    @foreach ($plane as $data)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 border-b text-gray-700">{{ $data->nomor_regis }} -
                                {{ $data->nomor_penerbangan }}</td>
                            <td class="px-4 py-3 border-b text-gray-700">{{ $data->kapasitas }}</td>
                            <td class="px-4 py-3 border-b text-gray-700">
                                @foreach ($data->plane_kelas as $kelas)
                                    {{ $kelas->nama_kelas }},
                                @endforeach
                            </td>
                            <td class="px-4 py-3 border-b text-gray-700">
                                <a href="{{ route('plane.show', $data->id_plane) }}"
                                    class="text-blue-600 hover:text-blue-800 font-semibold mr-3">Edit</a>
                                <form action="{{ route('plane.destroy', $data->id_plane) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Yakin ingin menghapus plane ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @else
                    <div class="flex items-center p-4 mb-4 text-sm text-red-800 bg-red-100 rounded-lg" role="alert">
                        ...
                        <span class="font-medium mr-2">Warning:</span>Data plane tidak tersedia.
                    </div>
                @endif
            </tbody>
        </table>
