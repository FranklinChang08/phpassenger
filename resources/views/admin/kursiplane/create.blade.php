<x-admin-layout>
    <div
        ...
        <h1 class="text-gray-800 font-bold">Create Kursi Plane</h1>
    ...
    <form action="{{ route('kursiplane.store') }}" method="POST"
        ...
        <div>
            <label for="total_kursi" class="block text-sm font-medium text-gray-700">Total Kursi</label>
            <div class="flex gap-4">
                <input required type="text" name="nama_kursi" id="nama_kursi" value="{{ old('nama_kursi') }}"
                    class="mt-1 w-16 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                @error('nama_kursi')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
                <input required type="text" name="total_kursi" id="total_kursi" value="{{ old('total_kursi') }}"
                    class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                @error('total_kursi')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
