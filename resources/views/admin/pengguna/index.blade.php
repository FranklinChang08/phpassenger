    <x-admin-layout>
        ...
        <div
            ...
            <h1 class="text-gray-800 font-bold">Pengguna List</h1>
        ...
        <div class="flex items-center gap-6 my-4">
            <a href="{{ route('pengguna.create') }}" class="font-bold bg-blue-600 px-4 py-2 rounded-md text-white">Tambah
                Pengguna</a>
            <form action="{{ route('pengguna.index') }}" method="GET" class="flex items-center rounded-lg gap-2">
        ...
        <table class="table-auto w-full bg-white rounded-lg shadow-md overflow-hidden">
            <thead class="bg-gray-100">
                <tr class="text-left">
                    ...
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Email</th>
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Nomor Telephone</th>
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Role</th>
                    <th class="px-4 py-2 border-b font-bold text-gray-800">Actions</th>
                </tr>
            </thead>
            <tbody>
                @if ($pengguna && $pengguna->count() > 0)
                    @php
                        $no = 1;
                    @endphp
                    @foreach ($pengguna as $data)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 border-b text-gray-700">{{ $data->role }}</td>
                            <td class="px-4 py-3 border-b text-gray-700">
                                <a href="{{ route('pengguna.show', $data->id_pengguna) }}"
                                    class="text-blue-600 hover:text-blue-800 font-semibold mr-3">
                                    Edit
                                </a>
                                <form action="{{ route('pengguna.destroy', $data->id_pengguna) }}" method="POST"
                                    class="inline-block"
                ...
                @else
                    <div class="flex items-center p-4 mb-4 text-sm text-red-800 bg-red-100 rounded-lg" role="alert">
                        ...
                        <span class="font-medium mr-2">Warning:</span>Data Kelas tidak tersedia.
                    </div>
                @endif
            </tbody>
        </table>
